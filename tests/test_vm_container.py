...
class TestDockerfile:
    ...
    def test_installs_cloud_image_utils(self):
        content = (VM_DIR / "Dockerfile").read_text()
        assert "cloud-image-utils" in content, \
            "cloud-init seed 作成に cloud-image-utils が必要です"

    def test_healthcheck_present(self):
        content = (VM_DIR / "Dockerfile").read_text()
        assert "HEALTHCHECK" in content, \
            "Dockerfile に HEALTHCHECK が必要です"
...
class TestEntrypoint:
    ...
    def test_exists(self):
        assert (VM_DIR / "entrypoint.sh").is_file(), \
            "vm/entrypoint.sh が存在しません"

    def test_is_executable(self):
        path = VM_DIR / "entrypoint.sh"
        assert path.stat().st_mode & 0o111, \
            "entrypoint.sh に実行権限が必要です"
    ...
    def test_passes_enable_kvm(self):
        ...
        assert "-enable-kvm" in content, \
            "entrypoint.sh で -enable-kvm を指定する必要があります"

    def test_vnc_flag_present(self):
        content = (VM_DIR / "entrypoint.sh").read_text()
        assert "-vnc" in content, \
            "entrypoint.sh で -vnc を指定する必要があります"

    def test_first_boot_handling(self):
        """初回起動時のイメージダウンロードロジックがあること。"""
        content = (VM_DIR / "entrypoint.sh").read_text()
        assert 'if [ ! -f "$VM_IMAGE" ]' in content or \
               'if [ ! -f "${VM_IMAGE}" ]' in content or \
               "if [ ! -f" in content, \
...
class TestCloudInit:
    ...
    def test_meta_data_has_instance_id(self):
        ...
        assert "instance-id" in data, \
            "meta-data に instance-id が必要です"

    def test_meta_data_has_hostname(self):
        path = VM_DIR / "cloud-init" / "meta-data"
        data = yaml.safe_load(path.read_text())
        assert "local-hostname" in data, \
            "meta-data に local-hostname が必要です"
...
class TestDockerCompose:
    ...
    def test_has_websockify_service(self):
        ...
        data = yaml.safe_load(path.read_text())
        services = data.get("services", {})
        assert "websockify" in services, \
            "docker-compose.yml に websockify サービスが必要です"

    def test_vm_has_kvm_device(self):
        content = (PROJECT_ROOT / "docker-compose.yml").read_text()
        assert "/dev/kvm:/dev/kvm" in content, \
            "vm サービスに /dev/kvm デバイスマウントが必要です"
...
class TestBackendDockerfile:
    ...
    def test_installs_dependencies(self):
        ...
        assert "uv sync" in content, \
            "Dockerfile で uv sync による依存インストールが必要です"

    def test_exposes_backend_port(self):
        content = (PROJECT_ROOT / "Dockerfile").read_text()
        assert "EXPOSE 8080" in content, \
            "Dockerfile に EXPOSE 8080 が必要です"
